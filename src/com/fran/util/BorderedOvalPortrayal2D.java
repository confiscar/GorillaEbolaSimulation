...
public class BorderedOvalPortrayal2D extends OvalPortrayal2D {
  ...
  public BorderedOvalPortrayal2D() {
    this(1.0D);
  }

  public BorderedOvalPortrayal2D(boolean filled) {
    this(1.0D, filled);
  }

  public BorderedOvalPortrayal2D(double scale) {
    this(scale, true);
  }

  public BorderedOvalPortrayal2D(Paint paint, Paint paintBorder) {
    this(paint, paintBorder, 1.0D);
  }

  public BorderedOvalPortrayal2D(double scale, boolean filled) {
    this(Color.gray, Color.black, scale, filled);
  }

  public BorderedOvalPortrayal2D(Paint paint, Paint paintBorder, double scale) {
    this(paint, paintBorder, scale, true);
  }

  public BorderedOvalPortrayal2D(Paint paint, Paint paintBorder, boolean filled) {
    this(paint, paintBorder, 1.0D, filled);
  }
  ...
  public void draw(Object object, Graphics2D graphics, DrawInfo2D info) {
    ...
    if (info.precise) {
      ...
      this.preciseEllipse.setFrame(
          info.draw.x - width / 2.0D, info.draw.y - height / 2.0D, width, height);
      if (this.filled) {
